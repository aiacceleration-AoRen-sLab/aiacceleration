...
class GradientComputation:
    ...
    def __init__(self, model, scale):
        ...
        # Determine the main device of the model
        if hasattr(model, 'hf_device_map') and 'model.embed_tokens' in model.hf_device_map:
            self.device = model.hf_device_map['model.embed_tokens']
        else:
            # Try to get the device of the first parameter in the model
            try:
                param_device = next(model.parameters()).device
                self.device = param_device
            except StopIteration:
                # If there are no parameters, use CPU as default device
