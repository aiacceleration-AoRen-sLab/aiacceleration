...
class D2Prune_OPT:
    ...
    # --- TensorBoard: Can be toggled via args / customize log directory ---
        ...
        if self.tb_enabled and (self.tb_writer is None):
            # Each process/experiment has independent run name (can be customized with model name)
            import datetime, os
            ts = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
            run_dir = os.path.join(self.tb_logdir, f"{self.tb_runname}-{ts}")
            self.tb_writer = SummaryWriter(log_dir=run_dir)

        # Convenient no-op wrapper to avoid null checks everywhere
    ...
    def check_outlier_quantile(mask, quantile=0.99):
        ...
        # Count elements greater than this quantile
...
# ========== MeZO dynamic sparsity method ==========
    ...
    def get_layer_dynamic_sparsity(self, subset, gpts_layers, wrapped_layers, dsm='owl', granularity='per-block'):
        ...
        if dsm == "owl":
            ...
            if granularity == 'per-block':
                ...
                self.block_sizes = []
                # self.block_importance = []  # New addition: Block importance weights (based on attention head contribution rate)
                for name in subset:
                    ...
                    # # SVD 
                    # U, S, VT = torch.linalg.svd(W_metric, full_matrices=False) # SVD decomposition of WS
                    ...
                    self.block_sizes.append(subset[name].weight.numel())  # Record parameter count
                ...
                # Map to [target_sparsity - lambda, target_sparsity + lambda] range
                target_sparsity = self.args.sparsity_ratio
                delta = (self.all_blocks_ratio - np.mean(self.all_blocks_ratio)) * self.args.Lambda * 2
                self.all_blocks_ratio = np.clip(target_sparsity + delta, 0.1, 0.95)  # Limit sparsity range
    ...
    def _compute_local_bounds(self, global_sparsity, layer_idx, num_layers, r_min, r_max):
        ...
        # Original bandwidth
        base_band = max(0.10, 0.30 * min(gs, 1.0 - gs))  # Slightly relaxed
        if num_layers <= 1:
            scale = 1.0
        else:
        ...
        # —— Minimum explorable bandwidth fallback (critical) ——
        MIN_BAND = 0.05  # At least allow ±5%
    ...
    def mezo_dsm(self, global_sparsity, inps, outs, attention_mask, position_ids, layer,
        ...
        dtype = torch.float32
        # —— Calculate adaptive upper/lower bounds for "this layer" (around global_sparsity) ——
        # ---- Add after variable initialization in mezo_dsm (if not present) ----
        ...
        # === In mezo_dsm(), after calculating layer_idx, define a tensorboard base tag ===
        base_tag = f"mezo/layer_{layer_idx}"

        # (Optional) Record initial r and dense baseline
        try:
            # Lightweight: only statistics/histogram, don't write large tensors
        ...
        def prune_and_loss(current_r, X, Y, seed_for_all):
            ...
            # Target global sparsity (scalar), compatible with list case
            ...
            for i, name in enumerate(subset_copy):
                ...
                if not math.isfinite(sp):
                    sp = float(global_sparsity)   # Or (r_min_local + r_max_local)/2
                ...
                if self.args.use_wanda_forward:
                    ...
                    try:
                        if name not in self.target_layer_names:   # Weight update branch
                            gpts_copy[name].fasterprune(sp, self.prune_n, self.prune_m)
                            gpts_copy[name].free()
                        else:                                     # Wrapped layer branch
                            wrapped_layers_copy[name].fasterprune(sp, self.prune_n, self.prune_m)
                            wrapped_layers_copy[name].free()
                    except Exception:
                        # Once pruning internally throws an error (including NaN/Inf), directly return huge loss to avoid polluting this round's estimation
            ...
            with torch.no_grad():
                ...
                # If output contains non-finite values, return huge loss
                ...
                if getattr(self.args, "sparse_reg", True):
                    ...
                    # Deviation between global weighted sparsity and target
        ...
        # ====== Evaluation function: given r, return average intra-layer MSE on a small batch of inputs ======
        def eval_avg_loss(current_r, max_eval_samples= min(16, inps.shape[0]), seed_for_all=self.args.seed):
            self._seed_all(seed_for_all)
            # Subsample to accelerate (can also use all inps)
        ...
        for epoch in range(epochs):
            ...
            total_r_loss = 0.0
            total_s_loss = 0.0
            # === Temporary accelerator when "can't learn anymore" ===
            lr_epoch, zo_eps_epoch, n_spsa_epoch = lr, zo_eps, n_spsa
            if prev_epoch_loss is not None:
                improve = prev_epoch_loss - 0.0  # Initialize with previous epoch mean
            # Real improve calculated below; placeholder here
            # ======= Batch version of SPSA gradient estimate =======
            batch_size = self.args.batch_size  # Can be set to self.args.batch_mezo
            ...
            # ====== Statistics improvement, "kick" if hardly decreasing ======
            ...
            try:
                ...
                # Don't record histograms too frequently; can record every few epochs or final round
            ...
            # Manual slight learning rate decay (optional)
            # lr *= 0.95
    ...
    def prune_llm(self, train_loader):
        ...
        for i in trange(len(self.layers), desc='Pruning Processing'):
            ...
            elif self.args.dsm == 'mezo': # Layer-by-layer sparsity acquisition
            ...
            elif self.args.dsm == "owl-mezo": # First layer, then block
                if self.args.granularity == 'per-block':
        ...
        if self.args.dsm == 'owl' or self.args.dsm == 'mezo':
            ...
            for i in range(len(self.layers)):
                ...
                if self.args.free:
                    ...
                    torch.cuda.empty_cache()
                try:
                    # If current mode is mezo or owl-mezo and layer r has been calculated
        ...
        if getattr(self, "tb_writer", None):
            ...
            # For long-running jobs that need continuous writing, can skip close. One-time scripts recommend close.
...
class Prune_OPT:
    ...
    def compute_sensitivity(S, top_ratio=0.05, mode="clamp"):
        """
        # Calculate sensitivity score based on eigenvalue spectrum
        Args:
            # S (torch.Tensor): Eigenvalue vector [d]
            # top_ratio (float): Take what proportion of eigenvalue energy ratio
            # mode (str): 'abs' or 'clamp'
        Returns:
            sensitivity (float)
        """
        if mode == "abs":
            ...
            S = torch.clamp(S, min=0.0)  # Clamp negative numbers to 0
        
        if S.sum() == 0:  # Avoid division by zero
            return 0.0
    ...
    def get_layer_dynamic_sparsity(self, subset, gpts, dsm='owl', granularity='per-block'):
        ...
        if dsm == "owl":
            ...
            if granularity == 'per-block':
                self.layer_outlier_ratios = []
                self.block_sizes = []
                # self.block_importance = []  # Block importance weights (based on attention head contribution rate)
                for name in subset:
...
# === PATCH 2: replace your mezo_dsm with this version ===
                ...
                if self.args.use_wanda_forward:
                    ...
                    try:
                        if self.args.prune_method == 'sparsegpt':
                            ...
                            # Fixed ADMM configuration, ensure L+/L- completely consistent
    ...
    def greedy_dsm(
        self,
        global_sparsity,                  # Target weighted sparsity (e.g., 0.5/0.6/0.7/0.8)
        inps, outs, attention_mask, position_ids, layer,
        subset, GPT,
        alpha=0.02,                       # Base step size coefficient, larger is faster but rougher
        r_min=0.10, r_max=0.98,           # Lower/upper bounds for each block
        eval_batch=16,                    # How many samples to use for evaluation per step
        lam_reg=0.0,                      # Light regularization (e.g., 1e-4), suppress putting all budget on one block
        max_steps=10000,                 # Maximum external steps, prevent infinite loop
        seed=None
    ):
        """
        # Block-wise Greedy Optimization (see Algorithm 1 you sent)
        Return: tensor of length n, representing sparsity r_i of each sub-block in this layer
        """
        ...
        # --- Parameter scale f_i of each sub-block and total scale F ---
        with torch.no_grad():
            f = torch.tensor([subset[name].weight.numel() for name in names], dtype=torch.float64, device=device)
        F = float(f.sum().item())

        # --- Initialize p and global progress P (per paper: p=0; here starting with r_min is more stable) ---
        p = torch.full((n,), float(r_min), dtype=torch.float64, device=device)
        P = float((p * f).sum().item() / F)                   # Current weighted sparsity
        target = float(global_sparsity)
        target_sum = target * F

        # --- Pre-compute dense output (CRN) ---
        dense_Y = torch.zeros_like(outs)
        for j in range(inps.shape[0]):
            with torch.no_grad():
                dense_Y[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask)[0]
        ...
        # --- Two small tools: evaluation, one-step pruning and loss calculation ---
        def _prune_and_loss(current_r, X, Y, seed_for_all):
            ...
            with torch.no_grad():
                sparse_Y = layer_copy(X, attention_mask=attention_mask)[0]
                # Standardized MSE, offset scale differences between different layers/samples
        ...
        # --- Base step size: α · (F/f_i) (large blocks take small steps, small blocks take large steps), plus an "equilibrium" discount (1 + p_i) ---
        def _base_step(i):
            # Avoid forever stacking on the same block: as p_i increases, step size gradually decreases
        ...
        # If starting point already meets target (extreme case when r_min is very large), directly fine-tune
        if (p * f).sum().item() >= target_sum - 1e-12:
            outer = max_steps  # Skip loop

        while outer < max_steps and (p * f).sum().item() < target_sum - 1e-12:
            outer += 1

            # Try one by one: increase p_i by one step, record error
            E = np.full((n,), np.inf, dtype=np.float64)
            step_cache = np.zeros((n,), dtype=np.float64)

            cur_sum = float((p * f).sum().item())       # = P * F
            need_sum = max(0.0, target_sum - cur_sum)   # How much "weighted sparsity" is still needed

            for i in range(n):
                if float(p[i]) >= r_max - 1e-12:   # Skip if reaching upper bound
                    continue

                raw = _base_step(i)
                # Maximum step size that doesn't exceed budget or upper bound
            ...
            # No updatable blocks, finish work
            if not np.isfinite(E).any():
                self.logger.info("[GREEDY] all blocks saturated or no feasible step; stop.")
                break

            # Select block j with minimum error
            j = int(np.nanargmin(E))
            # —— Actually execute update (still strictly adhere to budget and boundaries) ——
            ...
            # —— Accept and record (platform loss also updates best_p) ——
            best_loss = float(E[j])
            best_p = p.clone()
            ...
            # If this step has precisely hit the target, exit directly
            if target_sum - (p * f).sum().item() <= 1e-12:
                break

        # ===== Pull P precisely to target =====
        cur_sum = float((p * f).sum().item())
        gap = target_sum - cur_sum
        if abs(gap) > 1e-9:
            # Prioritize adjusting the last updated j; if not found, pick the block with largest capacity
            candidate_idxs = list(range(n))
            candidate_idxs.sort(key=lambda t: -float(f[t]))  # Large blocks first, fine-tuning has less impact
            picked = None
            for t in [j] + candidate_idxs if 'j' in locals() else candidate_idxs:
                # Can still add upward
                if float(p[t]) < r_max - 1e-9 and gap > 0:
                    picked = t; break
                # Can still subtract downward (usually not needed, because starting point is r_min)
        ...
        # Return (double->float, put back to CPU)
    ...
    def prune_llm(self, train_loader):
        ...
        for i in trange(len(self.layers), desc='Pruning Processing'):
            ...
            # whether to prune by dynamic sparsity-->get subset layer sparsity-->
            if self.args.dsm == 'owl': # Global sparsity acquisition
                if self.args.granularity == 'per-block':
                ...
                self.layer_r = self.greedy_dsm(
                    ...
                    alpha=0.02,              # Recommended starting point: 0.01~0.03
                    r_min=0.10, r_max=0.98,  # Consistent with MeZO
        ...
        if self.args.dsm == 'owl' or self.args.dsm == 'mezo' or self.args.dsm == 'greedy':
            ...
            for i in range(len(self.layers)):
                ...
                # 1. forward layer wrapper
                subset, gpts = self.forward_layer_wrapper(layer, inps, outs, attention_mask, position_ids, GPT)
                # mezo determines dynamic sparsity of each block
                if self.args.granularity == 'per-layer':
