...
# Set HuggingFace mirror (if HF_MIRROR is set, also set HF_ENDPOINT)
hf_mirror = os.getenv('HF_MIRROR', None)
if hf_mirror and 'HF_ENDPOINT' not in os.environ:
    os.environ['HF_ENDPOINT'] = hf_mirror
    print(f"[INFO] Setting HF_ENDPOINT to mirror: {hf_mirror}")
...
is_npu = False
if hasattr(args, 'device') and args.device and 'npu' in str(args.device).lower():
    is_npu = True
    try:
        import torch_npu
        if torch.npu.is_available():
            print(f"[INFO] NPU detected. Using device: {args.device}")
    except ImportError:
...
if args.free:
    ...
    # If NPU device is specified, use the specified device type
    if is_npu and hasattr(args, 'device') and args.device:
        device_type = args.device  # Use specified npu:0 etc.
    else:
        device_type = "auto"  # Default auto selection
    model, tokenizer = get_model(args.model, device_type=device_type, seq_len=args.seqlen)
model.eval()
args.seq_len = model.seq_len
model_name = args.model.split("/")[-1] if hasattr(args, 'model') and args.model else "Unknown"
